...
namespace sf
{
    ...
    ////////////////////////////////////////////////////////////////////////////////
    void RichText::Line::setCharacter(std::size_t pos, unsigned int character)
    {
        assert(pos < getLength());
    ...
    ////////////////////////////////////////////////////////////////////////////////
    unsigned int RichText::Line::getCharacterStyle(std::size_t pos) const
    {
        assert(pos < getLength());
    ...
    ////////////////////////////////////////////////////////////////////////////////
    unsigned int RichText::Line::getCharacter(std::size_t pos) const
    {
        assert(pos < getLength());
        sf::Text& text = m_texts[convertLinePosToLocal(pos)];
    ...
    {
        // Set text offset
        text.setPosition({ m_bounds.size.x, 0.f });

        // Update bounds
        float lineSpacing = std::floor(text.getFont().getLineSpacing(text.getCharacterSize())) * 1.f; // TODO: wire m_lineSpacingFactor
        m_bounds.size.y = std::max(m_bounds.size.y, lineSpacing);
        m_bounds.size.x += text.getGlobalBounds().size.x;
    }
    ...
    ////////////////////////////////////////////////////////////////////////////////
    std::vector<sf::String> explode(const sf::String& string, unsigned int delimiter)
    {
        if (string.isEmpty())
    ...
    RichText& RichText::operator << (const sf::String& string)
    {
        ...
        if (it != subStrings.end()) {
            ...
            m_bounds.size.y -= line.getGlobalBounds().size.y;

            // Append text
            line.appendText(createText(*it, *m_font));

            // Update bounds
            m_bounds.size.y += line.getGlobalBounds().size.y;
            m_bounds.size.x = std::max(m_bounds.size.x, line.getGlobalBounds().size.x);
        }

        // Append the rest of substrings as new lines
        while (++it != subStrings.end()) {
            Line line;
            line.setPosition({ 0.f, m_bounds.size.y });
            line.appendText(createText(*it, *m_font));
    ...
    ////////////////////////////////////////////////////////////////////////////////
    void RichText::setCharacter(std::size_t line, std::size_t pos, unsigned int character)
    {
        assert(line < m_lines.size());
    ...
    void RichText::setOutlineColor(sf::Color color)
    {
        ...
        m_currentStroke.outline = color;
        for (Line& line : m_lines)
            for (auto& t : const_cast<std::vector<sf::Text>&>(line.getTexts())) {
    ...
    ////////////////////////////////////////////////////////////////////////////////
    unsigned int RichText::getCharacterStyle(std::size_t line, std::size_t pos) const
    {
        assert(line < m_lines.size());
    ...
    ////////////////////////////////////////////////////////////////////////////////
    unsigned int RichText::getCharacter(std::size_t line, std::size_t pos) const
    {
        assert(line < m_lines.size());
    ...
    ////////////////////////////////////////////////////////////////////////////////
    sf::Text RichText::createText(const sf::String& string, const sf::Font& font) const
    {
        sf::Text text(font);
        text.setString(string);
